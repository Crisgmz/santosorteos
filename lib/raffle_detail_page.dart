...
class _RaffleDetailPageState extends State<RaffleDetailPage> {
  ...
  Future<void> _onConfirmar(Sorteo sorteo) async {
    ...
    try {
      // Tomar números disponibles y reservarlos en la BD
      final disponibles =
          await _repo.fetchAvailableNumbers(sorteo.id, _cantidad);
      ...
      await _sendWhatsAppMessage(
        sorteo,
        reservedNumbers: reservados,
        orderId: orderId,
      );

      setState(() {
        _feedback = '¡Boletos reservados! Redirigiendo a WhatsApp...';
      });
    } catch (e) {
      setState(() {
        _feedback = 'Error: $e';
  ...
  Future<void> _sendWhatsAppMessage(
    Sorteo sorteo, {
    List<int>? reservedNumbers,
    String? orderId,
  }) async {
    final currency = NumberFormat.currency(locale: 'es_DO', symbol: 'RD\$');
    final numeros = reservedNumbers?.map((n) => n.toString().padLeft(4, '0'));
    final numerosLinea =
        numeros != null && numeros.isNotEmpty ? numeros.join(', ') : 'N/D';
    final message =
        '''
🎟️ *NUEVA RESERVA DE BOLETOS*

*Sorteo:* ${sorteo.titulo}
*Cliente:* ${_nombreCtrl.text.trim()}
*Cédula:* ${_cedulaCtrl.text.trim()}
*Teléfono:* DO +1 ${_telefonoCtrl.text.trim()}
*Números:* $numerosLinea
*Orden:* ${orderId ?? 'N/D'}

*Cantidad:* $_cantidad boleto${_cantidad > 1 ? 's' : ''}
*Precio por boleto:* ${currency.format(sorteo.precioTicket)}
*Total:* ${currency.format(sorteo.precioTicket * _cantidad)}

*Banco:* ${_selectedBank!['name']}
*Cuenta:* ${_selectedBank!['account']}
*Titular:* $_currentAccountHolder

${_imageFileName != null ? '📸 *Comprobante adjunto:* $_imageFileName' : '⚠️ Sin comprobante'}
    ''';

    final encodedMessage = Uri.encodeComponent(message);
    final whatsappUrl =
        'https://api.whatsapp.com/send?phone=18496285498&text=$encodedMessage';
  ...
  Widget _inlineTicketTile(VerifiedTicket t) {
    ...
    final statusColor =
        isReserved ? const Color(0xFFF59E0B) : const Color(0xFF16A34A);
